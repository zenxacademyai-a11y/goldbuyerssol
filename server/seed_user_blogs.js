...
// Map the configs into BlogPost schema
const newBlogs = articleConfigs.map((cfg) => {
  ...
  console.log(`Successfully Synthesized [${cfg.id}] "${cfg.title}" - Word Count: ~${wordCount}`);
...
// Update database blogs array (We can keep the first two blogs or replace all of them with the 10 requested ones)
// The user requested to write a blog for each of the 10 titles, which implies we should make these 10 the primary database entries!
currentDb.blogs = newBlogs;

// Write back to db.json
fs.writeFileSync(DB_FILE, JSON.stringify(currentDb, null, 2), "utf-8");
console.log(`Seeded ${newBlogs.length} beautifully formatted, SEO, AEO, and GEO optimized articles into data/db.json!`);
